...
const RUN_STATUS: Record<string, { icon: React.ReactNode; label: string; className: string }> = {
  COMPLETED:  { icon: <CheckCircle2 className="h-4 w-4" />, label: 'Completed',       className: 'text-green-400' },
  PARTIAL:    { icon: <AlertTriangle className="h-4 w-4" />, label: 'Partial',        className: 'text-amber-400' },
  FAILED:     { icon: <XCircle className="h-4 w-4" />,      label: 'Failed',          className: 'text-red-400'   },
  PROCESSING: { icon: <Loader2 className="h-4 w-4 animate-spin" />, label: 'Processing', className: 'text-blue-400' },
}
    ...
    <div className="max-w-3xl">
      ...
      <div className="card mb-6">
        ...
        <div className="mb-5 grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <div className={`text-2xl font-bold ${totalBalance >= totalSalary ? 'text-green-400' : 'text-red-400'}`}>
        ...
        {Object.entries(chainBalances).filter(([, v]) => parseFloat(v) > 0).length > 0 && (
          <div className="mb-4 grid grid-cols-2 gap-1.5 sm:grid-cols-3">
            {Object.entries(chainBalances).filter(([, v]) => parseFloat(v) > 0).map(([chain, bal]) => (
              <div key={chain} className="rounded-lg bg-gray-800 px-3 py-2 text-center">
                <div className="text-xs font-medium text-white">${parseFloat(bal).toFixed(2)}</div>
                <div className="text-xs text-gray-500">{chain.replace('-', ' ').replace('TESTNET', 'Testnet')}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {needsCctp && (
          <div className="mb-4 rounded-lg bg-amber-900/20 px-4 py-2 text-xs text-amber-400">
      ...
      <h2 className="mb-3 text-sm font-semibold text-gray-300">Payroll History</h2>
      ...
      {loading ? (
        ...
        <div className="card py-10 text-center text-sm text-gray-500">No payroll runs yet.</div>
      ) : (
